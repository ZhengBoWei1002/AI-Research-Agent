...
- `Reviewer` checks evidence sufficiency after each `Researcher` pass
- the workflow records each review decision in `reflection_history`
- `reasoning_log` stores step-by-step routing and review reasoning
- `transition_history` keeps the workflow path traceable
- `latest_feedback` is passed back to `Researcher` during retry
...
- retries are capped by `research_max_iterations`
- `Reviewer` requests retry when evidence count or source diversity is too low
- `Researcher` broadens tool queries after review feedback
- once the retry limit is reached, `Reviewer` approves the current evidence set
- each retry is persisted in `reflection_count` and `reflection_history`

## Agent 协作流程

- `Supervisor` normalizes and accepts the incoming task
- `Planner` decomposes the objective into `ResearchTask` items
- `Researcher` gathers evidence and tool traces
- `Reviewer` decides whether to retry or continue
- `Writer` creates the current milestone report draft
...
- `START → Supervisor → Planner → Researcher ↔ Reviewer → Writer → END` workflow invocation
